...
public:
    ...
    void set_task_finish_spin_enabled(bool enabled) { task_finish_spin_enabled = enabled; }
    ...
    TaskManager(std::vector<list<int>> &tasks, int num_of_agents) : tasks(tasks), num_of_agents(num_of_agents) {
        ...
        agent_cooldown_steps.resize(num_of_agents, 0);
...
private:
    ...
    int num_of_agents;
    bool task_finish_spin_enabled = false;
    vector<int> agent_cooldown_steps;
