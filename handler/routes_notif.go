...
func RegisterNotifRoutes(api *gin.RouterGroup, db *gorm.DB) {
	...
	notifCompat := api.Group("/notif")
	{
		notifCompat.GET("/get_notification", notifH.GetNotificationCompat)
		notifCompat.POST("/read_notification", notifH.ReadNotificationCompat)
		notifCompat.POST("/del_notification", notifH.DeleteNotificationCompat)
	}
}
